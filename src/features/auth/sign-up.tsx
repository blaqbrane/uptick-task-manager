        ...
        <form onSubmit={ handleSubmit} className="md:w-[500px] shadow-md rounded-lg shadow-[grey] p-4">
         <h2 className="font-bold text-2xl text-center">SIGN UP</h2>
              <div className='flex flex-col mt-2 mb-3'>
                ...
                <button disabled={email.length < 5 || password.length < 6} className={`${email.length < 5 || password.length < 4 ? 'opacity-50 cursor-not-allowed' :'opacity-100'} bg-orange-500 text-white px-4 py-2 rounded-md `}>{isLoading ? <FiLoader className="animate-spin text-white"/> : 'Sign Up'}</button>
        </form>

        <p className="mt-3">Have an account already <span className="text-orange-500"><Link to='/signin'>Sign In</Link></span></p>
      </div>
      </section>
    )
  }
